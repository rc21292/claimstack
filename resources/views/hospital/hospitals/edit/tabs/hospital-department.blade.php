<form action="{{ route('hospital.hospitals.department', $hospital->id) }}" method="post" id="hospital-department-form"
    ...
    <div class="col-md-12">
                <a href="{{ route('hospital.hospitals.edit', $hospital->id) }}?show_doctor=1"> <button style="float:right;" type="button" class="btn btn-danger show-empanelment"> <i class="mdi mdi-plus"></i> Add Doctor </button></a>
    </div>
      ...
      <tbody>
        ...
        @foreach($hospital_department as $hod)
         <tr>
            ...
            <td scope="row">
               <a href="{{ url('hospital/hospitals/'.$hod->hospital_id.'/edit?show_doctor=1&doctor_id=').$hod->id }}" class="btn btn-primary"><i class="mdi mdi-pencil"></i></a>
               <button type="button" class="btn btn-danger" onclick="confirmDeleteDoctor({{$hod->id}})"><i class="uil uil-trash-alt"></i></button>
               <form id="delete-form{{ $hod->id }}" action="{{ route('hospital.hospitals.doctor-delete', $hod->id ) }}" method="POST">
                @csrf  
               </form>
            </td>
         </tr>
