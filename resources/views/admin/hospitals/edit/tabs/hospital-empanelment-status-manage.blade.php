            ...
            <tbody>
                @foreach ($empanelments as $empanelment)
                    <tr>
                        ...
                        <td scope="row">
                            <a href="{{ route('admin.hospitals.edit', $hospital->id) }}?company_id={{ $empanelment->id }}" class="btn btn-primary"><i class="mdi mdi-pencil"></i></a>
                            <button type="button" class="btn btn-danger"
                            onclick="confirmDelete({{ $empanelment->id }})"><i
                            class="uil uil-trash-alt"></i></button>

                            <form id='delete-form{{ $empanelment->id }}'
                                action='{{ route('admin.hospitals.hospital-delete', ['id' => $hospital->id, 'eid' => $empanelment->id]) }}'
                                method='POST'>
